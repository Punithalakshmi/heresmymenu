...
if (get_theme_mod('show_top_dishes',true) == true) {
	add_action('frozr_below_indexloop', 'best_selling_dishes', 10);
}
if (!function_exists ('best_selling_dishes') ) {
...
add_action('frozr_below_indexloop', 'latest_resturants', 10);
}
if (!function_exists ('latest_resturants') ) {
...
//home advance search
if (!function_exists ('home_advance_search') ) {
	...
	<div id="resturants_search_box">
		...
		<?php $front_src_trash_ordr = array('none'); if (isset($_COOKIE['frozr_user_src_type']) && $_COOKIE['frozr_user_src_type'] == "restaurants") {
			...
			home_advance_search_body($srcordr);
			}
			} else {
	...
	<?php $front_src_ordr_snd = array('catsearch','ingsearch','spysearch'); if (isset($_COOKIE['frozr_user_src_type']) && $_COOKIE['frozr_user_src_type'] == "restaurants" ) {
		...
		foreach($front_src_ordr_snd as $src_ordr) {
			home_advance_search_body($src_ordr);
		} ?>
...
add_action('before_first_sec','home_advance_search');
...
function home_advance_search_body($xn) {
