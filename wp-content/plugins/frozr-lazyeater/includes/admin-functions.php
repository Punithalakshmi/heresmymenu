...
function frozr_on_order_refund( $order_id, $old_status, $new_status ) {
	...
	if ($old_status == 'completed' && get_post_meta($order->id, '_payment_method', true) != 'cod') {
		$seller_profit = get_post_meta( $order_id, 'frozr_order_seller_profit', true );		
		...
		$seller_profit_val = (!empty ($seller_profit)) ? $seller_profit : $order_total;
		
		update_user_meta($order_post->post_author, "_restaurant_balance", ($seller_current_balance - $seller_profit_val) );
		$msg_args = apply_filters('frozr_minus_user_balance_message_args',array (
			...
			'amount' => wc_price($seller_profit_val),
			'new_sts' => $new_status,
...
add_action( 'show_user_profile','add_meta_fields', 20 );
add_action( 'edit_user_profile','add_meta_fields', 20 );
