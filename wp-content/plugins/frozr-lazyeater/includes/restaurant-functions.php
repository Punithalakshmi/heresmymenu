...
function store_rating_form( $seller, $orderid ) {
...
add_action('list_restaurants','frozr_stores_listing',10,5);
//Restaurants in loops
function restaurants_inloops($u) {
...
function restaurants_open_close($u, $lyt = true) {
...
function frozr_rest_status($u, $lyt = true) {
	$nx = restaurants_open_close($u, $lyt);
	return $nx[0];
}
//check if restaurant is open
function frozr_is_rest_open($u) {
	$nx = restaurants_open_close($u);
	...
	<?php
		...
		    <tr>
				<td class="dish-title">
					...
					<h6>
                        <!--<a href="#item-<?php //echo $post->ID; ?>" title="View <?php //_e('Quick View','frozr'); ?>" data-transition="fade" data-rel="popup">-->
                    <a title="View <?php _e('Quick View','frozr'); ?>" data-transition="fade" data-rel="popup">
                    <?php the_title(); ?>
                    </a></h6>
					<span class="product_detalis">
                        <?php frozr_limit_info( apply_filters( 'frozr_product_popup_short_description', $post->post_excerpt),70); ?>
                    </span>
					...
					<div data-history="false" data-role="popup" id="item-<?php echo $post->ID; ?>" class="dish-info <?php if ( $product->is_on_sale() ) {echo "del_pe";} ?>" data-position-to="window">
						...
						<div class="rest-cont-thumbnail" style="background: url('<?php echo $large_image_url[0]; ?>') no-repeat center center;background-size:cover;">
                        <div class="dish_pop_thumb">
                            <?php the_title(); ?><br /><?php woocommerce_template_loop_price(); ?>
                        </div>
                        </div>
						<div class="dish_quick_info">
							<?php do_action('frozr_before_product_quick_info'); ?>
							<div class="dish_veg_sp">
								<span><span class="<?php echo ($dish_type) ? $dish_type : 'veg'; ?>"></span>
                                <?php echo $dish_type_text;?></span>
								<span><span class="dish-is-spicy"></span><?php echo $dish_spicy_text; ?></span>
							</div>
							<div class="dish_det">
                                <?php the_title(); echo apply_filters('frozr_product_popup_description', $post->post_excerpt); ?>&nbsp;
                                <a href="<?php the_permalink() ?>" title="View <?php the_title_attribute(); ?>">
                                    <?php _e('View full details.','frozr'); ?>
                                </a>
                            </div>
							<?php if($dish_fat == 'yes') {?>
							<div class="dish-fat"><i class="fs-icon-heartbeat"></i>&nbsp;
                                <?php echo __('Fat','frozr') . ' &cong; ' . $dish_fat_rate . __(' Grams','frozr'); ?>
                            </div>
							<?php } ?>
							<?php if(get_the_term_list($product->ID, 'ingredient')) { 
							 echo '<div class="dish_ing_single"><i class="fs-icon-leaf"></i>&nbsp;';
                    ...
                    <form data-product_id="<?php echo $post->ID; ?>" enctype="multipart/form-data" method="post" class="cart ajax_lazy_submit" data-enhance="false" data-ajax="false">
...
function output_restaurant_settings() {
...
//get available seats
function rest_search_tables($xn, $xnx) {
...
function restaurant_email_form($nx, $admin = false) { ?>
