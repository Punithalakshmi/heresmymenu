 ...
 */
add_action( 'init', 'create_ingredients', 0 );
if (!function_exists ('create_ingredients') ) {
 ...
 */
add_action( 'init', 'create_delivery_location', 0 );
if (!function_exists ('create_delivery_location') ) {
	function create_delivery_location() {
 ...
 */
add_action( 'init', 'create_addresses_taxonomy', 0 );
if (!function_exists ('create_addresses_taxonomy') ) {
	function create_addresses_taxonomy() {
 ...
 */
add_action( 'init', 'create_rest_type', 0 );
if (!function_exists ('create_rest_type') ) {
	function create_rest_type() {
...
/** 
 * Seller Percentage (Depreciated)
 */
if (!function_exists ('frozr_get_seller_percentage') ) {
	function frozr_get_seller_percentage( $seller_id = 0 ) {
		$fle_option = get_option( 'fro_settings' );
		$global_percentage = (! empty( $fle_option['fro_general_percentage']) ) ? $fle_option['fro_general_percentage'] : 50;

		if ( ! $seller_id ) {
			return $global_percentage;
		}

		$seller_percentage = (int) get_user_meta( $seller_id, 'frozr_seller_percentage', true );
...
/**
 * Remove currency for shipping calculation
 */
 ...
 */
if (!function_exists ('add_delivery_fee') ) {
...
if (!function_exists ('lazyeater_dash_menu') ) {
	function lazyeater_dash_menu(){
...
add_action('frozr_add_user_top_menu_item', 'lazyeater_dash_menu');
...
if (!function_exists ('frozr_add_special_comments') ) {
	function frozr_add_special_comments() {
		...
		frozr_wp_radio (array ( 'id' => 'order_l_type', 'value' => 'delivery', 'options' => array('delivery' => array(__('Delivery','frozr'), 'fs-icon-motorcycle', orders_type_close_open($post, 'delivery')), 'pickup' => array(__('Pickup','frozr'), 'fs-icon-shopping-bag', orders_type_close_open($post, 'pickup')), 'dine-in' => array(__('Dine-in','frozr'), 'fs-icon-cutlery', orders_type_close_open($post, 'dine-in')), 'curbside' => array(__('Curbside','frozr'), 'fs-icon-car', orders_type_close_open($post, 'curbside')))));
...
add_action( 'woocommerce_add_order_item_meta', 'save_dish_special_comments', 10, 3 );
function save_dish_special_comments( $itemId, $values, $key ) {
...
function orders_type_close_open($post, $nx) {
	global $post;
...
//count user object
function count_user_object($sts, $type ="", $nx =""){
