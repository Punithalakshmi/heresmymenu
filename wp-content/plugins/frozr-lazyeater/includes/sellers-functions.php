...
//sellers lists body
function sellers_page_body() {
	
	global $post;
	
	sellers_page_nav(); ?>
	
	<div id="seller_mgs" class="common_pop" data-history="false" data-role="popup"><?php restaurant_email_form(0, true); ?></div>
		...
		if (!empty($sellers_results)) {
			if ($_GET['sellers'] == 'top-sellers') {
				...
				foreach ($sellers_results as $seller_result) {
					$com_ords = count_user_object('wc-completed','shop_order',$seller_result );
					if ($com_ords != 0) {
				...
				foreach ($topsellers as $topseller => $sellerv) {
					top_sellers_list_body($topseller);
					}
			} else {
			foreach ($sellers_results as $seller_result) {
					top_sellers_list_body($seller_result);
				}
			}
        } else { ?>
			<tr>
...
function top_sellers_list_body($xxn) {
	...
	$user_pro_orders = count_user_object('wc-processing','shop_order',$xxn );
	$user_com_orders = count_user_object('wc-completed','shop_order',$xxn );
	$user_coupons = count_user_object('publish','shop_coupon',$xxn );
	?>
	<tr>

		<?php do_action('frozr_before_seller_table_loop', $xxn); ?>
		
		<td><div id="seller_edit_pop_<?php echo $xxn; ?>" class="common_pop" data-history="false" data-role="popup"><?php seller_edit_form($xxn); ?></div> <a href="#seller_edit_pop_<?php echo $xxn; ?>" data-rel="popup" data-position-to="window"><strong><?php echo $user_info->user_login . $seller_store; ?></strong></a></td>
