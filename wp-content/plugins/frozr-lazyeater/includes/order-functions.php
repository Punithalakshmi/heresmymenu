    ...
    <ul class="order-statuses-filter">
        <li <?php echo $status_class == 'processing' ? ' class="active fs-icon-caret-up"' : ''; ?> >
            <a href="<?php echo add_query_arg( array( 'order_status' => 'processing' ), $orders_url ); ?>">
                <?php printf( __( 'Processing (%d)', 'frozr' ), count_user_object('wc-processing', 'shop_order') ); ?></span>
            </a>
        </li>
        <li <?php echo $status_class == 'completed' ? ' class="active fs-icon-caret-up"' : ''; ?> >
            <a href="<?php echo add_query_arg( array( 'order_status' => 'completed' ), $orders_url ); ?>">
                <?php printf( __( 'Completed (%d)', 'frozr' ), count_user_object('wc-completed', 'shop_order') ); ?></span>
            </a>
		</li>
		<li <?php echo $status_class == 'on-hold' ? ' class="active fs-icon-caret-up"' : ''; ?> >
            <a href="<?php echo add_query_arg( array( 'order_status' => 'on-hold' ), $orders_url ); ?>">
                <?php printf( __( 'On-hold (%d)', 'frozr' ), count_user_object('wc-on-hold', 'shop_order') ); ?></span>
            </a>
        </li>
        <li <?php echo $status_class == 'pending' ? ' class="active fs-icon-caret-up"' : ''; ?> >
            <a href="<?php echo add_query_arg( array( 'order_status' => 'pending' ), $orders_url ); ?>">
                <?php printf( __( 'Pending (%d)', 'frozr' ), count_user_object('wc-pending', 'shop_order')); ?></span>
            </a>
        </li>
        <li <?php echo $status_class == 'cancelled' ? ' class="active fs-icon-caret-up"' : ''; ?> >
            <a href="<?php echo add_query_arg( array( 'order_status' => 'cancelled' ), $orders_url ); ?>">
                <?php printf( __( 'Cancelled (%d)', 'frozr' ), count_user_object('wc-cancelled', 'shop_order') ); ?></span>
            </a>
        </li>
        <li <?php echo $status_class == 'refunded' ? ' class="active fs-icon-caret-up"' : ''; ?> >
            <a href="<?php echo add_query_arg( array( 'order_status' => 'refunded' ), $orders_url ); ?>">
                <?php printf( __( 'Refunded (%d)', 'frozr' ), count_user_object('wc-refunded', 'shop_order') ); ?></span>
            </a>
        </li>
        <li <?php echo $status_class == 'failed' ? ' class="active fs-icon-caret-up"' : ''; ?> >
            <a href="<?php echo add_query_arg( array( 'order_status' => 'failed' ), $orders_url ); ?>">
                <?php printf( __( 'failed (%d)', 'frozr' ), count_user_object('wc-failed', 'shop_order') ); ?></span>
            </a>
        </li>
	...
	<table class="orders_list_table ui-responsive table-stroke" data-role="table">
		<thead>
			<tr class="table_collumns">
				...
				<th data-priority="5"><?php _e( 'Via', 'frozr' ); ?></th>
...
global $post;
	...
	if ( $orders_query->have_posts() ) {
		while ($orders_query->have_posts()) { $orders_query->the_post();
			order_listing_body($post->ID, $ods);
		}
	} else { ?>
		<tr>
			<td colspan="7">
...
function frozr_get_seller_total_order( $order, $inc_fee = true, $inc_sub = true) {
	...
	if ( !empty ($fees = $order->get_fees()) && $inc_fee == true) {
...
//Order listing body
function order_listing_body($oid, $sts) {
	global $post;
	...
	<tr>
		...
		<td>
			<?php echo wc_price (frozr_get_seller_total_order($the_order), array('currency' => $the_order->get_order_currency())); ?>
		</td>
		<td>
			<?php if ( $the_order->payment_method_title ) {
			echo '<span class="meta">' . esc_html( $the_order->payment_method_title ) . '</span>';
			} ?>
		</td>
		<td>
 ...
 */
add_action( 'wp_ajax_frozr_add_order_note', 'add_order_note' );
 ...
 */
add_action( 'wp_ajax_frozr_delete_order_note', 'delete_order_note' );
...
function frozr_add_user_balance( $order_id ) {
	...
	if ( $order->status == 'completed' ) {
		...
		$percentage = frozr_get_seller_percentage( $order_post->post_author );
		$order_total = (frozr_get_seller_total_order($order) * $percentage)/100;

		$seller_profit_val = (!empty ($seller_profit['total_profit'])) ? $seller_profit['total_profit'] : $order_total;
		$website_profit = $order->frozr_get_seller_total_order($order) - $seller_profit_val;
		$user_current_balance = floatval(get_user_meta($order_seller,"_restaurant_balance", true));

		if (get_post_meta($order->id, '_payment_method', true) == 'cod' && '' == get_post_meta($order->id, 'frozr_order_website_fee_balance_detected', true)) {
