...
function frozr_summary_template($type = 'today', $start = '', $end = '', $user = '') {
	...
	if (is_super_admin()) {
		if (esc_attr($type) != 'custom') {
			$report_values = dash_total_sales($type, '', '', $user);
		} else {
			$report_values = dash_total_sales($type, $start, $end, $user);
		}
		if ($type == 'begging') {
		...
		if (esc_attr($type) != 'custom') {
			$report_values = dash_total_sales($type);
		} else {
			$report_values = dash_total_sales($type, $start, $end);
		}
		if ($type == 'begging') {
