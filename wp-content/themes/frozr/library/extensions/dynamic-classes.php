...
if ( function_exists( 'childtheme_override_body_class' ) )  {
	 ...
	 */
	function frozr_body_class( $c ) {
		global $wp_query, $current_user, $blog_id, $post, $taxonomy;
	
		if ( apply_filters('frozr_show_bc_wordpress', TRUE ) ) {
	        // It's surely a WordPress blog, right?
	        $c[] = 'wordpress';
	    }
	    
	    if ( apply_filters( 'frozr_show_bc_blogid', TRUE) ) {
	    	// Applies the blog id to BODY element .. blog-id1 for WordPress < 3.0
	    	$c[] = 'blogid-' . $blog_id;
	    }
	
		if ( apply_filters( 'frozr_show_bc_datetime', TRUE) ) {
	        // Applies the time- and date-based classes (below) to BODY element
	        frozr_date_classes( time(), $c );
	    }
	
	    if ( apply_filters( 'frozr_show_bc_contenttype', TRUE ) ) {
	    ...
	    if ( apply_filters( 'frozr_show_bc_singular', TRUE) ) {
	...
	if ( is_single() && apply_filters( 'frozr_show_bc_singlepost', TRUE ) ) {
			...
			if ( isset( $wp_query->post->post_date ) )
				frozr_date_classes( mysql2date( 'U', $wp_query->post->post_date ), $c, 's-' );
		...
		elseif ( is_author() && apply_filters( 'frozr_show_bc_authorarchives', TRUE ) ) {
		...
		elseif ( is_category() && apply_filters( 'frozr_show_bc_categoryarchives', TRUE ) ) {
		...
		elseif ( is_tag() && apply_filters('frozr_show_bc_tagarchives', TRUE ) ) {
		...
		elseif ( is_tax() && apply_filters( 'frozr_show_bc_taxonomyarchives', TRUE) ) {
			$c[] = 'taxonomy';
			$c[] = 'tax-' . $taxonomy;
			$c[] = $taxonomy . '-' . strtolower(frozr_get_term_name());
		}
	
		// Page author for BODY on 'pages'
		elseif ( is_page() && apply_filters( 'frozr_show_bc_pages', TRUE ) ) {
		...
		elseif ( is_search() && apply_filters( 'frozr_show_bc_search', TRUE ) ) {
			the_post();
		...
		if ( apply_filters( 'frozr_show_bc_loggedin', TRUE ) ) {
	 ...
	 // Paged classes; for page x > 1 classes of index and all post types etc.
		if ( isset( $post ) && apply_filters( 'frozr_show_bc_pagex', TRUE ) ) {
		...
		// And tada!
		return array_unique(apply_filters( 'frozr_body_class', $c )); // Available filter: frozr_body_class
	}
}

/**
 * Add Frozr body classes if child theme activates it
 */
function frozr_activate_body_classes() {
	if ( current_theme_supports ( 'frozr_legacy_body_class' ) ) {
		add_filter( 'body_class', 'frozr_body_class', 20 );
	}
	
	// Add browser CSS class to the end (queuing through priority) of the body classes 
	if ( apply_filters( 'frozr_show_bc_browser', TRUE ) ) {
		add_filter( 'body_class', 'frozr_browser_class_names', 30 ); 
	}
}
add_action( 'init', 'frozr_activate_body_classes' );



/**
 * frozr_browser_class_names function.
 */
function frozr_browser_class_names($classes) {
...
if (function_exists('childtheme_override_post_class'))  {
	 ...
	 */
	function frozr_post_class( $c ) {

		global $post, $frozr_post_alt, $frozr_content_length, $taxonomy;
		...
		$frozr_nextpaged = preg_match( '/<!--nextpage(.*?)-->/', $post->post_content );

		if( $frozr_nextpaged )
			$c[] = 'is-paged-excerpt';

		$frozr_excerpt_more = preg_match( '/<!--more(.*?)-->/', $post->post_content ); 
		
		// For posts displayed as full content
		if ( $frozr_content_length == 'full' && !has_excerpt() && !$frozr_excerpt_more && !$frozr_nextpaged )  
			$c[] = 'is-full';

		// For posts displayed as excerpts
		if ( $frozr_excerpt_more  || $frozr_content_length == 'excerpt' || has_excerpt() ) {
			$c[] = 'is-excerpt';
			if ( has_excerpt() ) {
				// For wp-admin Write Page generated excerpts
				$c[] = 'custom-excerpt';
			} elseif ( $frozr_excerpt_more ) {
				$c[] = 'moretag-excerpt';
			} else {
		...
		if ( $frozr_excerpt_more && is_single() ) {
		...
		frozr_date_classes( mysql2date( 'U', $post->post_date ), $c );
		...
		// And tada!
		return array_unique(apply_filters( 'frozr_post_class', $c )); // Available filter: frozr_post_class
	}
}

/**
 * Add Frozr post classes if child theme activates it
 */
function frozr_activate_post_classes() {
	if ( current_theme_supports ( 'frozr_legacy_post_class' ) ) {
		add_filter( 'post_class', 'frozr_post_class', 20 );
	}
}
add_action( 'init', 'frozr_activate_post_classes' );
 ...
 * @since 1.0
 */
function frozr_add_comment_class($classes) {
    global $comment, $post;

    // Add time and date based classes
    frozr_date_classes( mysql2date( 'U', $comment->comment_date ), $classes, 'thm-c-' );

    // Do not duplicate values
    return array_unique( $classes );
}

add_filter( 'comment_class', 'frozr_add_comment_class', 20 );
...
if ( function_exists( 'childtheme_override_date_classes' ) )  {
	 ...
	 */
	function frozr_date_classes( $t, &$c, $p = '' ) {
...
// Remember: Frozr, like The Sandbox, is for play.
